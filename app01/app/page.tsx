import Link from 'next/link';

export default function Home() {
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', maxWidth: '800px', margin: '0 auto' }}>
      <nav style={{ marginBottom: '20px', display: 'flex', gap: '20px', fontSize: '18px' }}>
        <Link href="/" style={{ fontWeight: 'bold', textDecoration: 'none', color: '#333' }}>Início</Link>
        <Link href="/produtos" style={{ textDecoration: 'none', color: '#0070f3' }}>Produtos</Link>
        <Link href="/sobre" style={{ textDecoration: 'none', color: '#0070f3' }}>Sobre Nós</Link>
        <Link href="/contato" style={{ textDecoration: 'none', color: '#0070f3' }}>Contato</Link>
      </nav>
      <hr style={{ border: '1px solid #eee', marginBottom: '20px' }} />
      <h1>ModaEstilo 2026</h1>
      <p>Bem-vindo à nossa loja! Confira as melhores tendências de roupas urbanas.</p>
      <div style={{ marginTop: '20px' }}>
        <img src="/banner.jpg" alt="Banner" style={{ width: '100%', borderRadius: '8px' }} />
      </div>
    </div>
  );
}